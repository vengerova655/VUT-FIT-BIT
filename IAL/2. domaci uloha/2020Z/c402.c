...
**/
	//kde mame vlozit uzol
	tBTNodePtr insert_Node = NULL;
	tBTNodePtr tmp = *RootPtr;
	while(tmp != NULL){
		insert_Node = tmp;
		//potrebujeme sa dostat do lavej vetvy
		if(Content < tmp->Cont){
			tmp = tmp->LPtr;
		}
		//potrebujeme sa dostat do pravej vetvy
		else if(Content > tmp->Cont)
		{
			tmp = tmp->RPtr;
		}
		//vkladany uzol uz existuje-> nic sa nedeje
		else
		{
			return;
		}
	...
	//nastavenie hodnot-> vklada sa vzdy list
	new_Node->Cont = Content;
	new_Node->LPtr = NULL;
	new_Node->RPtr = NULL;


	//vkladame ako novy koren
	if (insert_Node == NULL){
		*RootPtr = new_Node;
	}
	//vkladame na lavu stranu ak novy uzol ma mensiu hodnotu
	else if (Content < insert_Node->Cont)
	{
		insert_Node->LPtr = new_Node;
	}
	//vkladame na pravu stranu ak novy uzol ma vacsiu hodnotu
	else
	{
		insert_Node->RPtr = new_Node;
	}
}
...
**/
	...
	//ziskanie zasobnika s lavymi ptr
	Leftmost_Preorder(ptr,&s);
	while (!SEmptyP(&s))
	{
		ptr = STopPopP(&s);
		//potrebujeme spracovat vsetky uzly, preto aj doprava
...
** realizujte jako volání funkce BTWorkOut().
**/
	//vytvorenie zasobniku
	tStackP s;
	SInitP(&s);
	//koren
	tBTNodePtr ptr = RootPtr;
	//ziskanie hodnot na zasbnik
	...
	while (!SEmptyP(&s))
	{
		...
		//spracovanie
		BTWorkOut(ptr);
		//musime spracovat vsekty-> aj doprava
...
**/
	...
	tStackP sp;
	SInitP(&sp);
	//postorder-> 2 sa vracia k otcovi preto stack este dalsi s bool hodnotami
	tStackB sb;
	SInitB(&sb);
	//koren
	tBTNodePtr ptr = RootPtr;
	Leftmost_Postorder(ptr,&sp,&sb);
	while (!SEmptyP(&sp))
	{
		leftside = STopPopB(&sb);
		ptr = STopPopP(&sp);
		//prichadza zlava, pojde doprava
		if(leftside){
			SPushP(&sp,ptr);
			SPushB(&sb,false);
			Leftmost_Postorder(ptr->RPtr,&sp,&sb);
		}
		//prichadza sprava, spracuje uzol
		else
		{
			BTWorkOut(ptr);
		}
...
**/
	...
	do{
		if(*ptr == NULL){
			//beriem uzol zo zasobniku (ak tam nejaky je)
			if(!SEmptyP(&s)){
				*ptr = STopPopP(&s);
			}
		}
		...
		{
			//pravy uzol sa ulozi do zasobniku
			if((*ptr)->RPtr != NULL){
				SPushP(&s,(*ptr)->RPtr);
			}
			//pomocny na zrusenie
			tBTNodePtr tmpPtr = *ptr;
			//posun na dlasi lavy uzol
			(*ptr) = (*ptr)->LPtr;
			//rusenie potrebneho aktulaneho uzla
			free(tmpPtr);
		}
		
	}while (((*ptr)!=NULL)||(!SEmptyP(&s)));
}
