...
*/
	// osetrenie zadanie nealokovanej fronty
	if( q == NULL ) {
		queueError(QERR_INIT);
	}
	else
	{
		// nastavenie indexu prveho volneho prvku
		q->b_index = 0;
		// nastavenie indexu prveho prvku 
		q->f_index = 0;
		// nastavenie vsetkych hodnot v poli na '*'
...
*/
	// fronta -> po dosiahnuti maximalneho indexu indexujeme znovu od nuly
	// Aho**
	// ^  ^
	// F  B
	// Aho**
...
*/
	// prazdna jedine v pripade, ze prvy volny prvok == prvy prvok
	// True = 1 -> nenulova hodnota
...
*/
	// plna <=> dalsi index volneho prvku == idexu prveho prvku
	// True = 1 -> nenulova hodnota
...
*/
	// osetrenie  volania nad prazdnou frontou
	if ( queueEmpty(q) ) {
		queueError(QERR_FRONT);
	}
	else {
		//vratenie znaku zo zaciatku fronty
		*c = q->arr[q->f_index];
	}
}
...
*/
	//osetrenie volania nad prazdnou funkciou
	if	( queueEmpty(q) ) {
		queueError(QERR_REMOVE);
	}
	else{
		//posunutie indexu prveho  prvku na nasledujuci
...
*/
	// osetrenie volania nad volnou frontou
	if ( queueEmpty(q) ) {
		queueError(QERR_GET);
	}
	else{
		//vracanie odstranovaneho znaku
		queueFront(q,c);
		//odstranovanie znaku
		queueRemove(q);
	}
...
*/
	// osetrenie volania nad plnou frontou
	if ( queueFull(q) ) {
		queueError(QERR_UP);
	}
	else
	{
		// vlozenie znaku do prveho volneho prvku
		q->arr[q->b_index] = c;
		// obnovenie indexu noveho volneho prvku 
