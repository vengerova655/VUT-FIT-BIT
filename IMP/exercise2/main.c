...
/*                                                                            */
/*  Reseni vytvoril(a) a odevzdava: (Veronika Vengerova, xvenge01)                            */
/*                                                                            */
/******************************************************************************/
...
void PORTB_IRQHandler(void) {
	...
	if(PORTB->ISFR & BUTTON_LEFT_MASK){
		if(!(GPIOB->PDIR & BUTTON_LEFT_MASK)){
			beep();
		}
			PORTB->ISFR=BUTTON_LEFT_MASK;
	}

	if(PORTB->ISFR & BUTTON_UP_MASK){
		if(!(GPIOB->PDIR & BUTTON_UP_MASK)){
			flash();
		}
			PORTB->ISFR=BUTTON_UP_MASK;
	}

	if(PORTB->ISFR & BUTTON_DOWN_MASK | PORTB->ISFR & BUTTON_CENT_MASK | PORTB->ISFR & BUTTON_RIGHT_MASK){
